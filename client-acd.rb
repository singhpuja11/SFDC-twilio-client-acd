...
configure do
  ...
  @conn = Mongo::Client.new(mongohqdbstring)
...
get '/websocket' do 
  ...
  request.websocket do |ws|
    ...
    ws.onopen do
      ...
      mongoagents.update_one({_id: clientname} , { "$set" =>   {status: "LoggingIn",readytime: Time.now.to_f  },  "$inc"  =>  {:currentclientcount => 1}} , {upsert: true})
      settings.sockets << ws     
    end
    ...
    response = Twilio::TwiML::Response.new do |r|  
        if dialqueue  #If this variable is set, we have no agents to route to
            ...
            r.Dial(:timeout=>"10", :action=>"/handledialcallstatus", :callerId => callerid)  do |d|
                ...
                sidinsert = mongocalls.update_one({_id: sid},  agentinfo, {upsert: true})
...
post '/track' do
    ...
    mongoagents.update_one({_id: from} , { "$set" =>   {status: status,readytime: Time.now.to_f}})
...
post '/request_hold' do
    ...
    customer_call.update_one(:url => "#{request.base_url}/hold",
                 :method => "POST")  
...
post '/request_unhold' do
    ...
    call.update_one(:url => "#{request.base_url}/send_to_agent?target_agent=#{from}",
                 :method => "POST")  
    puts call.to
end
...
def getlongestidle (callrouting, mongoagents) 
   ...
   mongoreadyagent =  mongoagents.find( { "$query" => { "$or" => queryfor } , "$orderby" => {readytime: 1}  } ).first
     ...
     qsize =  account.queues.get(queueid).current_size
      ...
      if topmember #only check for availible agent if there is a caller in queue
        ...
        if bestclient
          ...
          mongoagents.update_one({_id: bestclient} , { "$set" =>   {status: "DeQueing" }  } )   
